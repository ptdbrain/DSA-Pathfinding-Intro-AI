...
roleToggle.addEventListener("change", function () {
  ...
  if (isAdmin) {
    ...
    // Reset chỉ các biến trạng thái, giữ lại đường cấm và vật cản
    isBlockMode = false;
...
// Thêm hàm kiểm tra node hợp lệ
function isValidNode(node) {
  return node && typeof node.lat === "number" && 
         typeof node.lon === "number" && 
         !isNaN(node.lat) && !isNaN(node.lon);
}

// Cập nhật hàm drawPath
function drawPath(path) {
  ...
  const latlngs = path.map(id => {
    ...
    if (!isValidNode(node)) {
      console.error(`Invalid node in path: ${id}`);
      return null;
    }
  ...
  if (latlngs.length < 2) {
    console.error("Not enough valid points to draw path");
    return;
  }
...
// Xử lý sự kiện reset
function resetMap() {
  // Reset các biến trạng thái chung
  selectedPoints = [];
  startPoint = null;
  isDrawing = false;
  isBlockMode = false;
  ...
  if (isAdmin) {
    // Admin reset - xóa tất cả
    blockedEdges = [];
    bannedLines = [];
    
    // Xóa các marker vật cản
    ...
    // Guest reset - chỉ xóa đường đi và điểm đã chọn
    map.eachLayer(function(layer) {
      if (layer instanceof L.Polyline && layer.options.color === "green") {
        // Xóa đường đi màu xanh
        map.removeLayer(layer);
      }
      else if (layer instanceof L.CircleMarker && layer.options.color === "red") {
        // Xóa điểm đã chọn màu đỏ
        map.removeLayer(layer);
      }
    });
...
// Cập nhật xử lý click trên bản đồ
map.on("click", function(e) {
  ...
  if (selectedPoints.length < 2) {
    ...
    if (selectedPoints.length === 2) {
      ...
      loadingDiv.innerHTML = "Đang tìm đường...";
